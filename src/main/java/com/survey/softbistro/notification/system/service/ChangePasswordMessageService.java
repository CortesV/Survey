...
@Service
public class ChangePasswordMessageService extends SurveyMessageService {
	...
	public void send(int page) {
		try {
			...
			List<RegistrationMessage> messages = iSurveyMessage.getEmailOfNewPassword(page);
			...
			for (int emailIndex = 0; emailIndex < messages.size(); emailIndex++) {
				...
				log.info(String.format("Change password: %s", messages.get(emailIndex).getClientEmail()));
			}
